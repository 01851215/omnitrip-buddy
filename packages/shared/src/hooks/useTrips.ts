...
export function useDreamTrips() {
  ...
  const createDreamTrip = useCallback(async (title: string, description: string, coverImage?: string): Promise<boolean> => {
    ...
    const { error } = await supabase.from("dream_trips").insert({
      user_id: user.id, title, description, cover_image: coverImage ?? null,
    });
    if (!error) refresh();
    return !error;
  }, [user, refresh]);
...
export interface DestinationCoord {
  id: string; name: string; country: string; lat: number; lng: number; arrivalDate: string;
}
...
export function useAllDestinations() {
  ...
  useEffect(() => {
    ...
    const toCoords = (rows: any[]): DestinationCoord[] =>
      rows.filter((d: any) => d.lat != null && d.lng != null)
        .map((d: any) => ({ id: d.id, name: d.name ?? "", country: d.country ?? "", lat: d.lat, lng: d.lng, arrivalDate: d.arrival_date ?? "" }));
    ...
    (async () => {
      ...
      if (results.length === 0) {
        ...
        if (trips && trips.length > 0) {
          const { data: fallback } = await supabase.from("destinations")
            .select("id, name, country, lat, lng, arrival_date")
            .in("trip_id", trips.map((t) => t.id))
            .order("arrival_date", { ascending: true });
          results = toCoords(fallback ?? []);
        }
      }
