...
export function FlightBookingModal({ deal, onClose, userId, tripId }: FlightBookingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md bg-surface rounded-t-3xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom">
        ...
        {step === "confirm" && (
          <div className="px-5 py-4 space-y-3">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={onClose} className="flex-1 text-xs font-medium text-text-muted bg-cream border border-cream-dark py-2.5 rounded-xl">
                Cancel
              </button>
              <button
                onClick={() => setStep("details")}
                className="flex-1 text-xs font-semibold text-white bg-primary py-2.5 rounded-xl"
              >
                Continue →
              </button>
            </div>
        ...
        {step === "details" && (
          <div className="px-5 py-4 space-y-3 max-h-[60vh] overflow-y-auto">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] text-text-muted font-medium uppercase tracking-wide">First Name</label>
                <input
                  value={form.firstName}
                  onChange={(e) => set("firstName", e.target.value)}
                  placeholder="John"
                  className="w-full mt-1 text-xs bg-cream border border-cream-dark rounded-lg px-3 py-2 outline-none focus:border-primary"
                />
              </div>
              <div>
                <label className="text-[10px] text-text-muted font-medium uppercase tracking-wide">Last Name</label>
                <input
                  value={form.lastName}
            ...
            <div>
              <label className="text-[10px] text-text-muted font-medium uppercase tracking-wide">Email</label>
              <input
                type="email"
                value={form.email}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] text-text-muted font-medium uppercase tracking-wide">Date of Birth</label>
                <input
                  type="date"
              ...
              <div>
                <label className="text-[10px] text-text-muted font-medium uppercase tracking-wide">Gender</label>
                <select
                  value={form.gender}
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={() => setStep("confirm")} className="flex-1 text-xs font-medium text-text-muted bg-cream border border-cream-dark py-2.5 rounded-xl">
                ← Back
              </button>
              <button
                onClick={handleBook}
                disabled={!isFormValid || loading}
                className="flex-1 text-xs font-semibold text-white bg-primary py-2.5 rounded-xl disabled:opacity-50"
              >
        ...
        {step === "success" && (
          <div className="px-5 py-6 text-center space-y-4">
            ...
            <button onClick={onClose} className="w-full text-xs font-semibold text-white bg-primary py-3 rounded-xl">
              Done
            </button>
          </div>
        )}
        ...
        {step === "error" && (
          <div className="px-5 py-6 text-center space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setStep("details")} className="flex-1 text-xs font-medium text-text-muted bg-cream border border-cream-dark py-2.5 rounded-xl">
                Try Again
              </button>
              <a
